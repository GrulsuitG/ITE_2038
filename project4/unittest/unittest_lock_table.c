...
transfer_thread_func(void* arg)
{
	...
	for (int i = 0; i < TRANSFER_COUNT; i++) {
		...
		accounts[destination_table_id][destination_record_id]
			+= money_transferred;
	//	if(destination_lock->pointer == NULL)
		//	printf("dest\n");
	//	if(source_lock->pointer == NULL)
		//	printf("src\n");
		/* Release lock!! */
...
int main()
{
	...
	//pthread_create(&scan, 0, scan_thread_func,NULL);
...
int main(){
	...
	pthread_create (&th1, 0 , func1, NULL);
//	sleep(1);
	pthread_create (&th2, 0 , func2, NULL);
